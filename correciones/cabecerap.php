                  ...
                  <td width="422" height="124" align="left" valign="top" background="<?php echo BASE_REL;?>img/barra_opciones_3.png" style="background-repeat: no-repeat"><table width="404" height="93" border="0" cellpadding="0" cellspacing="0">
                      <tr>
                              ...
                              <td height="25"><table width="298" border="0" align="center" cellpadding="0" cellspacing="0">
                                  <tr>
                                    <td width="215"><div align="center">
                                        <input name="txtEmail2" id="txtEmail2" type="text" class="opaco" size="26" style="border: 0px solid #1e1977; width:190px;" onFocus=" opacar(this.id,1); completartxt1(this.id,'INGRESAR EMAIL'); " onBlur=" opacar(this.id,0); completartxt0(this.id,'INGRESAR EMAIL');" />
                                    </div></td>
                                    <td width="83"><div align="right"><a href=""><img src="<?php echo BASE_REL;?>img/enviar1.jpg" width="75" height="29" border="0" /></a></div></td>
                                  </tr>
                              </table></td>
                            </tr>
                            <tr>
                        ...
                        <td width="83"><table width="90" border="0" align="center" cellpadding="0" cellspacing="0">
                          <tr>
                            <td width="85"><div align="center"><a href="http://www.mereferrals.seattlewebmedia.net/inicio_EN.php"><img src="<?php echo BASE_REL;?>img/ver_ingles.jpg" width="54" height="35" border="0" /></a></div></td>
                          </tr>
                          <tr>
          ...
          <tr>
                  ...
                  <td height="44"><table width="962" height="44" border="0" align="center" cellpadding="0" cellspacing="0">
                      <tr>
                        <td width="974" style="background-repeat: no-repeat"><table width="960" height="44" border="0" cellpadding="0" cellspacing="0">
                            <tr>
                              <td width="161" background="<?php echo BASE_REL;?>/img/bt_buscar_negocio_off.png"><a href="#" onclick="buscar1('negocio'); return false"><img src="<?php echo BASE_REL;?>img/bt_buscar_negocio_off.png" name="bt_b_negocio" id="bt_b_negocio" width="161" height="44" border="0" /></a></td>
                              <td width="143"><div align="left"><a href="#" onclick="buscar1('cupon'); return false"><img id="bt_b_cupon" name="bt_b_cupon" src="<?php echo BASE_REL;?>img/bt_buscar_cupon_on.png" width="142" height="44" border="0" /></a></div></td>
			...
			$hasta = $desde + $mostrar - 1;?>
              	...
              	<div id="cnt_al_bq" style="border:solid 0px #B8DC7C; z-index:25">&nbsp;</div>
                        ...
                        <td width="390"><a href="#" onclick="formPrueba('si');"><strong class="slogan_pie_registro">PROBANDO</strong></a></td>
                      </tr>
                  </table></td>
                </tr>
            </table></td>
...
<div id="pnlPrueba" class="pnlPrueba" style="display:none; position: fixed;
	z-index:3000;   left: 50%;    top: 50%;    height: 400px;    margin-top: -200px;   width: 450px;  margin-left: -225px;" >
<img src="http://<?php echo $_SERVER['HTTP_HOST'] ?>/img/cargando.gif">Cargando....
</div>
...
function frmRegNegocio(accion){
	...
	xajax_regNegocio(null,null,null,null,null,null,null,null,null,null,null,null,null,null);
	//$nombre,$email,$clave,$rclave,$zip,$negocio,$ndireccion,$nciudad,$nzip,$nest,$nfono,$nemail,$ncat,$nscat
	}else{
	document.getElementById('fondotransparente').style.display='none';
...
function loguear()
{
	...
	if(form.txtUsuario.value == "") {
		innerHtml('msjErrorLogeo','Ingrese su usuario');		
		form.txtUsuario.focus(); 
		return false;
	}
	if(form.txtClave.value == ""){
		innerHtml('msjErrorLogeo','No ha escrito su clave');		
		form.txtClave.focus();
		return false;
	}	
...
var error="";
var frm=null;
if(frm_a=='RUsuario') frm=document.frmRUsuario;
else
	if(frm_a=='RAsociado') frm=document.frmRAsociado;
	else
		if(frm_a=='RNegocio') frm=document.frmRNegocio;
...
for(i=0;i<sexos.length;i++){
  	...
  	if(validarTexto(nombre,5) == false) {
		innerHtml(msj,'Ingrese su Nombre Correctamente');		
		frm.txtNombre.focus(); 
		return false;
	}
	if(validarCorreo(email,10)==false) {
		innerHtml(msj,'Ingrese un email correcto');		
		frm.txtEmail.focus(); 
		return false;
	}
	if(validarClave(clave,rclave,5) == false) {
		innerHtml(msj,'Ingrese su clave y repitalo exactamente igual');
    	frm.txtRClave.value='';	
		frm.txtClave.focus(); 
		return false;
	}
	if(validarTexto(zip,5) == false) {
		innerHtml(msj,'Ingrese un codigo postal Correcto');
    	frm.txtZip.focus(); 
		return false;
	}
	if(validarTexto(sexo,1) == false) {
		innerHtml(msj,'Seleccione su sexo');
    	return false;
	}
	if(frm_a=='RUsuario'||msj=='msjErrorRAsociado2'||msj=='msjErrorRNegocio2')
	if(!terminos.checked){
		innerHtml(msj,'Debe aceptar los terminos y condiciones');
		return false;	
	}
	
	if(frm_a=='RUsuario'){
		innerHtml(msj,'');
		xajax_guardarUsuario(nombre,email,clave,rclave,zip,sexo);
	}
    else
	if(frm_a=='RAsociado'){
		if(msj=='msjErrorRAsociado1'){
			...
			innerHtml(msj,'');
			xajax_guardarAsociado(nombre,email,clave,rclave,zip,sexo,paypal);
		}
	}
	else
	if(frm_a=='RNegocio')
		...
		if(msj=='msjErrorRNegocio2'){
			if(validarTexto(negocio,10) == false){
			innerHtml(msj,'Ingrese el nombre de su Negocio');
			frm.txtNegocio.focus(); 
			return false;
			}
			if(validarTexto(ndireccion,10) == false){
			innerHtml(msj,'Ingrese la direccion de su negocio');
			frm.txtNDireccion.focus(); 
			return false;
			}
			if(validarTexto(nciudad,10) == false){
			innerHtml(msj,'Ingrese la Ciudad donde queda su negocio');
			frm.txtNCiudad.focus(); 
			return false;
			}
			if(validarTexto(nzip,10) == false){
			innerHtml(msj,'Ingrese el codigo postal de su negocio');
			frm.txtNZip.focus(); 
			return false;
			}
			if(validarTexto(nestado,10) == false){
			innerHtml(msj,'Selecione un estado');
			frm.txtNEstado.focus(); 
			return false;
			}
			if(validarTexto(nfono,10) == false){
			innerHtml(msj,'Ingresa el numero de Telefono de tu negocio');
			frm.txtNFono.focus(); 
			return false;
			}
			if(validarCorreo(nemail,10) == false){
			innerHtml(msj,'Ingrese el correo de su negocio');
			frm.txtNEmail.focus(); 
			return false;
			}
			if(validarTexto(ncategoria,10) == false){
			innerHtml(msj,'Selecione una Categoria');
			frm.txtNCategoria.focus(); 
			return false;
			}
			if(validarTexto(nsubcategoria,10) == false){
			innerHtml(msj,'Selecione una  subcategoria');
		...
		alert("ok")
			xajax_guardarNegocio(nombre,email,clave,rclave,zip,negocio,ndireccion,nciudad,nzip,nestado,nfono,nemail,ncategoria,nsubcategoria);
		}
					
} 


function formPrueba(accion){
	if(accion=='si'){	
	document.getElementById('fondotransparente').style.display='block';
	document.getElementById('pnlPrueba').style.display='block';
	xajax_registrarse(null,null);
	}else{
	document.getElementById('fondotransparente').style.display='none';
	document.getElementById('pnlPrueba').style.display='none';		
	}
}
